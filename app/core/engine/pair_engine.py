...
class TradeDecisionAgent:
    ...
    def __init__(self, pair: str):
        ...
        self.signal_filter = MLSignalFilter(pair=self.pair, allow_missing_prediction=False)
        self.strategy_selector = StrategySelection(pair=self.pair)
    ...
    def decide(
        ...
        allowed = self._filter_signal(normalized_signal, market_context, prediction)
        if not allowed:
    ...
    def _filter_signal(self, signal: Dict[str, Any], market_context: Dict[str, Any], prediction: Dict[str, Any]) -> bool:
        merged_context = dict(market_context)
        merged_context["prediction"] = prediction
        try:
            return bool(self.signal_filter.filter(signal, merged_context))
        except Exception:
            return False
